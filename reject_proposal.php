...
// Atualiza o status da proposta para 'rejeitada'
$sql = "UPDATE proposals SET status = 'rejected' WHERE id = ? AND babysitter_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ii", $proposal_id, $babysitter_id);

if ($stmt->execute()) {
    echo "Proposta rejeitada com sucesso.";
    header("Location: proposals.php");  // Redireciona para a página de propostas
    exit();
} else {
    echo "Erro ao rejeitar proposta: " . $stmt->error;
}
